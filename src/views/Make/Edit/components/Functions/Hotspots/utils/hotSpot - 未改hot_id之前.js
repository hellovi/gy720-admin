...
export default function hotspotsInit(that){
    window.krpanoplugin.prototype.adddesignhotspot = function(hotspotdata){
      ...
      // var copy = JSON.parse(JSON.stringify(hotspotdata))
     	...
     	var ondelete = "js(!window.krpanoDeleteHotspot || window.krpanoDeleteHotspot(" + hotspotdata.hot_id + "," + hotspotdata.scene_id + "," + hotspotdata.pano_id + "));";
   ...
   */
     ...
     * 删除热点
     */
   window.krpanoDeleteHotspot = function (hot_id, scene_id, pano_id) {
      const param = `?pano_id=${pano_id}&scene_id=${scene_id}`
      Http.delete(`/user/scenehotspot/${hot_id}${param}`)
        .then(() => {
          window.__krpano.removehotspot("hotspot_" + hot_id);
          window.__krpano.removelayer("hotspot_txt_" + hot_id);
          window.__krpano.hotspots[hot_id] = null;
        })
   }
